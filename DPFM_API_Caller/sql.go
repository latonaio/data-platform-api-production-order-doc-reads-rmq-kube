...
import (
	dpfm_api_input_reader "data-platform-api-production-order-doc-reads-rmq-kube/DPFM_API_Input_Reader"
	dpfm_api_output_formatter "data-platform-api-production-order-doc-reads-rmq-kube/DPFM_API_Output_Formatter"
	"fmt"
...
func (c *DPFMAPICaller) HeaderDoc(
	...
	groupBy := "\nGROUP BY ProductionOrder, DocType, DocIssuerBusinessPartner "

	rows, err := c.db.Query(
		`SELECT
    ProductionOrder, DocType, MAX(DocVersionID), DocID, FileExtension, FileName, FilePath, DocIssuerBusinessPartner
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_production_order_header_doc_data
		` + where + groupBy + `;`)
	if err != nil {
...
func (c *DPFMAPICaller) ItemDoc(
	...
	if input.HeaderDoc.ItemDoc.ProductionOrderItem != nil {
		where = fmt.Sprintf("%s\nAND ProductionOrderItem = %d", where, *input.HeaderDoc.ItemDoc.ProductionOrderItem)
	}
	...
	groupBy := "\nGROUP BY ProductionOrder, ProductionOrderItem, DocType, DocIssuerBusinessPartner "

	rows, err := c.db.Query(
		`SELECT
    ProductionOrder, ProductionOrderItem, DocType, MAX(DocVersionID), DocID, FileExtension, FileName, FilePath, DocIssuerBusinessPartner
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_production_order_item_doc_data
		` + where + groupBy + `;`)
	if err != nil {
		*errs = append(*errs, err)
		return nil
